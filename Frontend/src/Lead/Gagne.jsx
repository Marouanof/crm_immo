...
const Gagne = () =>{
    ...
    const [leadsGagnes, setLeadsGagnes] = useState([]);
    ...
    const navigate = useNavigate();
     useEffect(() => {
        const fetchLeadsGagnes = async () => {
            try {
                // const res = await axios.get('http://127.0.0.1:8000/lead/leads/');
                // setLeadsGagnes(res.data.filter(lead => lead.statut === 'Gagné'));
                ...
                if( role === "commercial"){
                  ...
                  setLeadsGagnes(allData.filter(b => b.statut === "Gagné"));
    ...
    const villes = [...new Set(leadsGagnes.map(lead => lead.quartiers[0].ville))];
    const degres = ["Haut", "Moyen", "Faible"];
    // const quartiers = [...new Set(data.map(lead => lead.quartiers[0]?.nom))];
    const quartiers = [
        ...new Set(
            leadsGagnes.flatMap(lead => lead.quartiers.map(q => q.quartier)) // q.quartier est un tableau
        )
    ];
    const statuts = [...new Set(leadsGagnes.map(lead => lead.statut))];
    const etatBiens = [...new Set(leadsGagnes.map(lead => lead.etat_bien))];
    const typesTransaction = [...new Set(leadsGagnes.map(lead => lead.type_transaction))];
    // const chambres = [...new Set(data.map(lead => lead.nbr_chambre))];
    const chambres = [
        ...new Set(
            leadsGagnes.flatMap(lead => lead.quartiers.map(q => q.nbr_chambre)).flat() // si c’est un tableau
        )
    ];

    // Filtrer les données
    const filteredData = leadsGagnes.filter((lead) => {
          ...
          <tbody>
            {currentRows.map((lead, index) => (
              <tr key={index}>
                ...
                <td>
                  <span className="badge-gagne">{lead.statut}</span>
                </td>
               </tr>
            ))}
          </tbody>
