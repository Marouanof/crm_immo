...
const LeadsARappeler = ({ onRefresh }) => {
    ...
    const getMotifStyle = (motif) => {
        const styles = {
            'recherche_de_bien': { backgroundColor: '#DBEAFE', color: '#1E3A8A' }, // Cyan clair / Cyan foncé
            'rdv_annule': { backgroundColor: '#FEF3C7', color: '#B45309' },        // Orange clair / Orange foncé
            'rdv_non_honore': { backgroundColor: '#FEE2E2', color: '#B91C1C' },    // Rouge clair / Rouge foncé
            'rappel_demande': { backgroundColor: '#EDE9FE', color: '#5B21B6' },    // Violet clair / Violet foncé
            'reflexion': { backgroundColor: '#CCFBF1', color: '#0F766E' },         // Turquoise clair / Turquoise foncé
            'negociation': { backgroundColor: '#F5D0FE', color: '#A21CAF' },       // Rose-violet clair / Violet foncé
            'telephone_eteint': { backgroundColor: '#F3F4F6', color: '#374151' },  // Gris clair / Gris foncé
        };
        return styles[motif] || {};
    };
    const fetchLeads = async () => {
        try {
            ...
            if( role === "commercial"){
              ...
              setAllLeads(allData.filter(b => b.statut === "A rappeler"));
              setDisplayedLeads(allData.filter(b => b.statut === "A rappeler"));
              // Récupérer les biens associés pour chaque lead
                ...
                setAllLeads(res.data);
                setDisplayedLeads(res.data.filter(lead => lead.statut === "A rappeler"));// Récupérer les biens associés pour chaque lead
                const biensData = {};
                await Promise.all(res.data.map(async (lead) => {
                    try {
                        const biensRes = await axios.get(`${apiUrl}/lead/associated-biens/${lead.id}/`);
            ...
            // // Récupérer les biens associés pour chaque lead
            // const biensData = {};
            // await Promise.all(allLeads.map(async (lead) => {
            //     try {
